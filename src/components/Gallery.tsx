...
const galleryImages = [
  ...
  { id: 3,  src: "/images/bennett_placement.png",            thumb: "/images/bennett_placement.png",            alt: "Bennett University Placement Drive", category: "Placement Drives",     caption: "Placement drive and interaction at Bennett University" },
...
export default function Gallery() {
  ...
  const prev = () => { if (lightboxIndex === null) return; setLightboxIndex((lightboxIndex - 1 + filtered.length) % filtered.length); };
  const next = () => { if (lightboxIndex === null) return; setLightboxIndex((lightboxIndex + 1) % filtered.length); };

  return (
    <section id="gallery" ref={ref} className="relative py-24 md:py-32 bg-white overflow-hidden">
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((img, i) => (
              <motion.div
                ...
                className="relative group mb-3 rounded-2xl overflow-hidden cursor-pointer border border-slate-200 hover:border-blue-200 hover:shadow-md transition-all duration-300"
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-4 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                  <div className="flex items-center gap-1.5 mb-1">
                    <ZoomIn size={13} className="text-blue-200 flex-shrink-0" />
                    <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-blue-200">{img.category}</span>
                  </div>
        ...
        {lightboxIndex !== null && (
          ...
          >
            ...
            >
              <button
                onClick={closeLightbox}
                className="absolute -top-11 right-0 text-slate-400 hover:text-white transition-colors"
              >
                <X size={24} />
              </button>
              ...
              <div className="relative rounded-2xl overflow-hidden border border-white/10">
                ...
                />
                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/75 to-transparent px-6 py-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-blue-300 mb-1">
              ...
              <div className="flex items-center justify-between mt-4">
                <button onClick={prev} className={btn.ghost + " flex items-center gap-1.5 !text-slate-300"}>
                  <ChevronLeft size={18} /> Previous
                </button>
                <span className="text-slate-500 text-sm tabular-nums">
                  {lightboxIndex + 1} / {filtered.length}
                </span>
                <button onClick={next} className={btn.ghost + " flex items-center gap-1.5 !text-slate-300"}>
